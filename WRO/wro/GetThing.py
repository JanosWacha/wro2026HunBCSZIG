...
#    print('Zold:', np.sum(color[:, :N_oszlop//2]==2), np.sum(color[:, N_oszlop//2:]==2))
    ...
    piros_van = (color[:, oszlopszelesseg:(N_oszlop-1)*oszlopszelesseg] == 1).sum () > N_sor * (N_oszlop-2) * 0.5
    zold_van = (color[:, oszlopszelesseg:(N_oszlop-1)*oszlopszelesseg] == 2).sum () > N_sor * (N_oszlop-2) * 0.5
    return piros_van, zold_van
    

if __name__ == '__main__':
    with open('colors.txt', 'wt') as f:
        while True:
            p, z = get_thing()
            if p:
                print('Piros')
